...
// const phns = document.querySelector('input[name="phone"]');
...
// 					$('input[name="phone"]').mask('+7 (999)-999-99-99');
// 				} else {
// 					$('input[name="phone__number"]').mask('8 (999)-999-99-99');
// 				}
// 			}
		...
		function K(r, n, t, xt, _t) {
			...
			function Xn(n, t, r, e, i) {
				...
				if (o)
					...
					for (; c < a[xi.l]; c++) h ? n[0][u](a[c], r, d) : n[f](a[c], r)
			}
...
const maxQuantity = (el, isDecrease) => {
			...
			if (!isEmpty(parentBodyOfStep)) {
				if (Cookies.get('deliveryID') !== undefined) {
					...
					if (Cookies.get('deliveryID') === '1' && !$('#urface').prop('checked')) {
						$('label[for=payment_3]').remove();
						$('label[for=payment_4]').remove();
						$('input#payment_1').prop('checked', true);
						$(`.${$('input#payment_1').parents('label').data('toggle_element')}`).addClass('active');
					}
					if (Cookies.get('deliveryID') === '1' && $('#urface').prop('checked')) {
						$('label[for=payment_4]').remove();
						$('input#payment_1').prop('checked', true);
...
if (window.miniShop2 !== undefined) {
	...
	miniShop2.Callbacks.add('Order.submit.response.success', 'orders_submit_ok', function(responseMinishop) {
		...
		if ($('#payment_4').is(':checked')) {
			...
			const invoiceId = +`${d}${m}${yyyy}${responseMinishop.data.msorder}`;
			const amount = +$('#ms2_order_cost').text().replace(' ', '');
			
			$.get("https://imperial.foxyland.online/core/components/epay/payment.php", { invoiceId, amount })
			.done(function(response) {
				...
				const createPaymentObject = function(auth, invoiceId, amount) {
					const paymentObject = {
							invoiceId,
							backLink: `${window.location.origin}/?msorder=${responseMinishop.data.msorder}`,
							failureBackLink: `${window.location.origin}/?error=${responseMinishop.data.msorder}`,
							postLink: `${window.location.origin}/core/components/epay/succsess.php`,
							failurePostLink: `${window.location.origin}/core/components/epay/error.php`,
							language: "RU",
				...
				// console.log(createPaymentObject(auth, invoiceId, amount));
			...
			const docName = pathCheck.split('/')[1];
