...
class msOrderHandler implements msOrderInterface
{
    ...
    public function submit($data = array())
    {
        ...
        $pathCheck = $response['data']['pathCheck'];
            ...
            if (
                ...
                if ($this->config['json_response']) {
                    ...
                    if (is_array($response)) {
                        $response['data']['pathCheck'] = $pathCheck;
                    } else {
                        $response = json_decode($response, true);
                        $response['data']['pathCheck'] = $pathCheck;
                    }
                    ...
                    if (!empty($response['data']['redirect'])) {
                        ...
                        $this->modx->sendRedirect(
                            $this->modx->context->makeUrl(
                                ...
                                array('msorder' => $response['data']['msorder'], 'pathCheck' => $pathCheck)
                            )
                        );
                    } else {
                    ...
                    return $this->success('', array('msorder' => $msOrder->get('id'), 'pathCheck' => $pathCheck));
                }
            } else {
                if ($this->ms2->config['json_response']) {
                    return $this->success('', array('msorder' => $msOrder->get('id'), 'pathCheck' => $pathCheck));
