...
class officeAuthController extends officeDefaultController
{
    ...
    public function defaultAction()
    {
        ...
        if (!$this->modx->user->isAuthenticated($this->modx->context->key)) {
            ...
            } elseif (!empty($_SESSION['HybridAuth']['error'])) {
                $pls['error'] = $_SESSION['HybridAuth']['error'];
            } elseif (!empty($_SESSION['Office']['Auth']['error']) && !empty($_SESSION['HybridAuth']['error'])) {
                $pls['error'] = $_SESSION['Office']['Auth']['error'] . '<br/>' . $_SESSION['HybridAuth']['error'];
            }
            unset($_SESSION['Office']['Auth']['error'], $_SESSION['HybridAuth']['error']);
    ...
    public function formRegister(array $data)
    {
        ...
        // Check password
        $password = trim(@$data['password']);
        if (!empty($password) || !$this->modx->getOption('office_auth_activation')) {
        ...
        // Дополнительные поля, при обновлении пакета Office может затереться
        // Check inn
        $inn = '';
        if (isset($data['inn'])) {
            $inn = trim(@$data['inn']);
            if (empty($inn)) {
                return $this->error('Не заполнено поле БИН');
            }
            $exists = $this->modx->getCount('modUserProfile', array('inn' => $inn)) ||
                $this->modx->getCount('modUser', array('inn' => $inn));
            if ($exists) {
                return $this->error('Пользователь с таким БИН уже существует');
            }
        }
        // Check company name
        $companyName = '';
        if (isset($data['company_name'])) {
            ...
            if (empty($companyName)) {
                return $this->error('Не заполнено поле Наименование компании');
            }
            $exists = $this->modx->getCount('modUserProfile', array('company_name' => $inn)) ||
                $this->modx->getCount('modUser', array('company_name' => $inn));
            if ($exists) {
                return $this->error('Компания с таким названием уже существует');
            }
        }
    ...
    public function Login(array $data)
    {
        ...
        if ($user = $this->modx->getObject('modUser', $c)) {
            ...
            if ($activate === true) {
                ...
                $lifetime = (int)$this->modx->getOption('session_cookie_lifetime', null, 0);
    ...
    protected function _resetPassword($username, $password = '', $tpl = '')
    {
        ...
        */
        ...
        if ($mode == 'phone') {
            ...
            $pls['hash'] = $activationHash;
            $pls['action'] = 'auth/login';
            $pls['emailRaw'] = rawurlencode($email);
        }
    ...
    protected function _createUser(array $data)
    {
        ...
        if (!$this->modx->getOption('office_auth_activation') && !empty($data['password'])) {
            $params['specifiedpassword'] =
            $params['confirmpassword'] = $data['password'];
        }
        $response = $this->office->runProcessor('auth/create', $params);
        ...
        return $this->modx->getOption('office_auth_activation') || empty($data['password'])
            ? $this->_resetPassword($data['username'], $data['password'], $this->config['tplRegister'])
            : $this->formLogin(array(
                'username' => $data['username'],
                'password' => $data['password'],
                'csrf' => $this->office->getCsrfToken(),
            ));
    }
